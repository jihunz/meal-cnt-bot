...
class EventService:
    ...
    def should_skip_meal_count(self):
        """식사 인원 계산을 건너뛰어야 하는지 확인 (공휴일/워크샵 등)"""
        # 공휴일 확인
        is_holiday, holiday_info = self.is_holiday()
        if is_holiday:
            print(f'[{self.now}] 오늘은 공휴일입니다: {holiday_info}')
            return True
        
        # 워크샵 등 기타 일정 확인
        if self.check_workshop_event():
            print(f'[{self.now}] 오늘은 워크샵이 예정되어 있습니다.')
            return True
